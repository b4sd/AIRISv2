...
api_key = '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'
client = Groq(
            api_key=api_key,
        )
...
def split_text(text, max_words_per_line=20):
    ...
    for sentence in sentences:
        ...
        if word_count > max_words_per_line:
            ...
            for i in range(0, word_count, max_words_per_line):
...
# def split_text(text, max_words_per_line=10):
#     sentences = re.split(r'(?<=[.!?])\s+', text.strip())
#     lines = []
#     current_line = []
#     current_word_count = 0
        ...
        messages=[
            {
                ...
                "content":  """Bạn là một trợ lý ảo giúp sửa lỗi chính tả. Hãy sửa lỗi chính tả cho đoạn văn bản được đưa vào.
                            ...
                            Có thể có một số dòng không có ý nghĩa, hãy xóa những dòng đó dựa vào ngữ cảnh những câu xung quanh. 
                            Trả về cả phần văn bản không bị sửa. Văn bản đã sửa bắt buộc phải có xuống dòng giống văn bản gốc.
                            Chỉ được trả về phần văn bản đã sửa. Tuyệt đối không giải thích thêm, không dài dòng.
...
def process_plumber(file_path):
    ...
    with pdfplumber.open(file_path) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            header_height = 50
            footer_height = 50
            cropped_page = page.within_bbox((x0, y0 + header_height, x1, y1 - footer_height))
            lines = cropped_page.extract_text().splitlines()
            if len(lines) == 0:
                continue
            chars = sorted(cropped_page.chars, key=lambda c: (c["top"], c["x0"]))  
            ...
            for line, current_top in line_positions:
                if is_section_header(line, cropped_page):
                    continue
                end_gap = cropped_page.width - max(
            ...
            # print("new page")
            page_content = ""
            for i, (line, current_top) in enumerate(line_positions):
                ...
                previous_top = current_top
                if is_chapter(line, cropped_page):
            ...
            while True:
                try:
                    page_content = re.sub(r"[■◆▣◈⊗⊕☉⚙♆♨࿊᚛᚜ꓷƕʬʭꝎꜾ⁂⸘⥉↯⤜]", "", page_content)
                    page_content = fix_typo(page_content)
                    page_content = split_text(page_content)
                    # print("content:", page_content)
                    # page_content = ""
                    pages.append(page_content)
                    break  # Exit loop if successful
                except Exception as e:  # Catch and store the exception
                    retries += 1
                    if retries >= max_retries:
                        raise RuntimeError(f"fix_typo failed after {max_retries} retries") from e
...
sections, pages = process_plumber(file_path)
for page_number, page in enumerate(pages):
    # if page_number > 270 and page_number < 280:
    print(page_number)
    print(page, '\n\n')

# create blocks for file
# blocks, chapters, page_to_block = blockify(sections)
'''