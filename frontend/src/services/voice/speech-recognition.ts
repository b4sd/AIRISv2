...
export class SpeechRecognitionService implements VoiceController {
  ...
  private onStateChange?: (state: { isListening: boolean; confidence: number; error?: string }) => void;
  ...
  private initialize(): void {
    ...
    if (!SpeechRecognition) {
      ...
      this.onError?.('Trình duyệt không hỗ trợ nhận dạng giọng nói. Vui lòng sử dụng Chrome, Edge, hoặc Safari.');
      return;
    }
    ...
    this.announceToScreenReader('Hệ thống nhận dạng giọng nói đã sẵn sàng. Nhấn phím Space để bắt đầu nghe.');
  }
  ...
  private setupRecognition(): void {
    ...
    this.recognition.onerror = (event) => {
      ...
      switch (event.error) {
        ...
        case 'language-not-supported':
          errorMessage = 'Ngôn ngữ tiếng Việt không được hỗ trợ trên thiết bị này.';
          break;
      }

      this.onStateChange?.({ isListening: false, confidence: 0, error: errorMessage });
    ...
    this.recognition.onend = () => {
      ...
      if (this.confidence > 0) {
        this.announceToScreenReader(`Đã nhận dạng lệnh với độ tin cậy ${Math.round(this.confidence * 100)}%.`);
      }
    };
  }
  ...
  public startListening(): void {
    ...
    try {
      ...
      this.onError?.('Không thể bắt đầu nhận dạng giọng nói. Vui lòng thử lại.');
    }
  }
  ...
  public async parseNaturalLanguage(text: string): Promise<CommandIntent> {
    ...
    const patterns = [
      // Navigation commands
      { pattern: /^(mở|đọc|xem)\s+(sách|cuốn)\s*(.+)$/i, action: 'open_book', extract: 3 },
      { pattern: /^(chuyển|đi|tới)\s+(trang|trang số)\s*(\d+)$/i, action: 'navigate_page', extract: 3 },
      { pattern: /^(chuyển|đi|tới)\s+(chương|chương số)\s*(\d+)$/i, action: 'navigate_chapter', extract: 3 },
      { pattern: /^(trang|sang)\s+(tiếp theo|sau)$/i, action: 'next_page' },
      { pattern: /^(trang|về)\s+(trước|trước đó)$/i, action: 'previous_page' },
      
      // Reading commands
      { pattern: /^(đọc|bắt đầu đọc)\s*(to|lớn|cho tôi nghe)?$/i, action: 'start_reading' },
      { pattern: /^(dừng|tạm dừng|ngừng)\s*(đọc)?$/i, action: 'pause_reading' },
      { pattern: /^(tiếp tục|đọc tiếp)$/i, action: 'resume_reading' },
      { pattern: /^(đọc|nói)\s*(nhanh|chậm)\s*(hơn|lên|xuống)?$/i, action: 'adjust_speed' },
      { pattern: /^(thay đổi|đổi)\s*(giọng|voice)$/i, action: 'change_voice' },
      
      // Note commands
      { pattern: /^(ghi chú|note|viết|lưu)\s*:?\s*(.+)$/i, action: 'take_note', extract: 2 },
      { pattern: /^(xem|hiện|hiển thị)\s*(ghi chú|note)$/i, action: 'show_notes' },
      { pattern: /^(tìm|tìm kiếm)\s*(ghi chú|note)\s*(.+)$/i, action: 'search_notes', extract: 3 },
      { pattern: /^(xóa|xoá)\s*(ghi chú|note)\s*(cuối|gần nhất)$/i, action: 'delete_last_note' },
      
      // Summary commands
      { pattern: /^(tóm tắt|tổng kết)\s*(trang|trang này)$/i, action: 'summarize_page' },
      { pattern: /^(tóm tắt|tổng kết)\s*(chương|chương này)$/i, action: 'summarize_chapter' },
      { pattern: /^(tóm tắt|tổng kết)\s*(sách|cuốn sách|toàn bộ)$/i, action: 'summarize_book' },
      { pattern: /^(đọc|nghe)\s*(tóm tắt|bản tóm tắt)$/i, action: 'read_summary' },
      
      // Library commands
      { pattern: /^(xem|hiện|hiển thị)\s*(thư viện|sách)$/i, action: 'show_library' },
      { pattern: /^(thêm|tải|upload)\s*(sách|tệp)$/i, action: 'add_book' },
      { pattern: /^(đánh dấu|bookmark)\s*(trang|trang này)$/i, action: 'bookmark_page' },
      { pattern: /^(đi|tới|chuyển)\s*(bookmark|đánh dấu)$/i, action: 'go_to_bookmark' },
        ...
        if (extract && match[extract]) {
          switch (action) {
            ...
            case 'search_notes':
              ...
              break;
            case 'adjust_speed':
              parameters.direction = match[1].includes('nhanh') ? 'faster' : 'slower';
              break;
          }
        }
  ...
  public onStateChanged(callback: (state: { isListening: boolean; confidence: number; error?: string }) => void): void {
    this.onStateChange = callback;
  }

  public onCommandReceived(callback: (command: string, intent?: CommandIntent) => void): void {
  ...
  public setupKeyboardShortcuts(): void {
    ...
    document.addEventListener('keydown', (event) => {
      ...
      if (event.ctrlKey && event.code === 'KeyH') {
        event.preventDefault();
        this.announceToScreenReader('Các lệnh giọng nói có sẵn: Mở sách, Đọc to, Tóm tắt, Ghi chú, Chuyển trang. Nhấn Space để bắt đầu nghe.');
      }
    });
  }
