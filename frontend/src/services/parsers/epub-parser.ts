...
export class EPUBParser {
  ...
  private async extractMetadata(book: any): Promise<Partial<BookMetadata>> {
    try {
      ...
      return {
        ...
        language: metadata.language || this.detectLanguage(metadata.title, metadata.description),
  ...
  private extractISBN(metadata: any): string | undefined {
    if (metadata.identifier) {
      if (typeof metadata.identifier === 'string') {
        return metadata.identifier.includes('isbn') ? metadata.identifier : undefined;
      }
      if (Array.isArray(metadata.identifier)) {
        ...
        return isbn ? (typeof isbn === 'string' ? isbn : isbn.value) : undefined;
      }
    }
    return undefined;
  }

  private async extractContent(book: any): Promise<{ content: string; chapters: Chapter[] }> {
      ...
      for (let i = 0; i < spine.length; i++) {
        ...
        try {
          ...
          if (chapterText.trim()) {
            ...
            const estimatedPages = Math.max(1, Math.ceil(chapterText.length / 2000)); // ~2000 chars per page
  ...
  async extractChapterContent(file: File, chapterIndex: number): Promise<string> {
    try {
      const arrayBuffer = await file.arrayBuffer();
      const book = ePub(arrayBuffer);
      await book.ready;
      ...
      if (chapterIndex < 0 || chapterIndex >= spine.length) {
        throw new Error(`Chapter ${chapterIndex} does not exist. EPUB has ${spine.length} chapters.`);
      }
      ...
      throw new Error(`Failed to extract chapter ${chapterIndex}: ${error.message}`);
    }
  }
