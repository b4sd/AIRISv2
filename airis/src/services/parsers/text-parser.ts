...
export class TextParser {
  async parseFile(file: File): Promise<Book> {
    try {
      ...
      const bookData = createBookFromUpload(
        metadata.title || file.name.replace(/\.(txt|md)$/i, ''),
  ...
  private extractMetadata(file: File, content: string): Partial<BookMetadata> {
  ...
  async extractSection(file: File, startLine: number, endLine: number): Promise<string> {
    try {
      const content = await this.readFileContent(file);
      const lines = content.split('\n');
      ...
      throw new Error(`Failed to extract lines ${startLine}-${endLine}: ${error.message}`);
    }
  }
  ...
  async getLineCount(file: File): Promise<number> {
    try {
      ...
      throw new Error(`Failed to get line count: ${error.message}`);
    }
  }
  ...
  async parseMarkdown(file: File): Promise<Book> {
    ...
    const bookData = createBookFromUpload(
      metadata.title || file.name.replace(/\.md$/i, ''),
      metadata.author || 'Unknown Author',
      content,
      'txt',
      {
  ...
  private extractMarkdownMetadata(content: string): Partial<BookMetadata> {
    const lines = content.split('\n');
    let title: string | undefined;
    let author: string | undefined;
